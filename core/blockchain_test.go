...
func TestFastVsFullChains(t *testing.T) {
	...
	ancientDb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), rawdb.NewMemoryDatabase(), t.TempDir(), "", false, false, false, false)
	if err != nil {
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	makeDb := func() ethdb.Database {
		db, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), rawdb.NewMemoryDatabase(), t.TempDir(), "", false, false, false, false)
		if err != nil {
			t.Fatalf("failed to create temp freezer db: %v", err)
		}
		return db
...
func testInsertKnownChainData(t *testing.T, typ string) {
	...
	// Import the shared chain and the original canonical one
	chaindb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), rawdb.NewMemoryDatabase(), t.TempDir(), "", false, false, false, false)
	if err != nil {
		t.Fatalf("failed to create temp freezer db: %v", err)
	}
	defer chaindb.Close()
...
func TestTransactionIndices(t *testing.T) {
	...
	for _, l := range limit {
		frdir := t.TempDir()
		ancientDb, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), rawdb.NewMemoryDatabase(), frdir, "", false, false, false, false)
		if err != nil {
			t.Fatalf("failed to create temp freezer db: %v", err)
		}
...
func TestTxIndexer(t *testing.T) {
	...
	for _, c := range cases {
		frdir := t.TempDir()
		db, _ := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), rawdb.NewMemoryDatabase(), frdir, "", false, false, false, false)
