...
func isBlockKey(key, value []byte) bool {
	...
	case bytes.Equal(key, headBlockKey):
		return true
	// HeadFastBlockHash
	case bytes.Equal(key, headFastBlockKey):
		return true
	default:
		return false
	}
...
func SplitBlockDatabaseV2(db ethdb.Database, blockdb ethdb.KeyValueStore) error {
	fmt.Println("begin migrate")
